// /src/features/cubesim/CubeSimulatorClient.tsx
...
import styles from '@/app/cubesim/page.module.css';
...
// --- 메인 클라이언트 컴포넌트 ---
export default function CubeSimulatorClient() {
        ...
        <div className={styles.mainContainer}>
          ...
          <section className={styles.cubeSimulator}>
            <div className={styles.cubeContent}>
              <h1 className={styles.titleRow}><span className={`${styles.cubeIcon} ${styles.blackCube}`} />잠재능력</h1>
              <p>현재 등급: {sim.currentGrade}</p>
              <p>{sim.failText}</p>
              <button className={styles.button} onClick={() => { sim.setIsAdditionalMode(false); sim.handleCubeReroll(); }}>
                큐브 사용하기
              </button>
            </div>
            <div className={styles.additionalCubeContent}>
              <h2 className={styles.titleRow}><span className={`${styles.cubeIcon} ${styles.additionalCube}`} />에디셔널</h2>
              <p>현재 에디셔널 등급: {sim.additionalCurrentGrade}</p>
              <p>{sim.additionalFailText}</p>
              <button className={styles.button} onClick={() => { sim.setIsAdditionalMode(true); sim.handleCubeReroll(); }}>
                큐브 사용하기
              </button>
            </div>
          </section>
            ...
            {sim.cubeResults.after.length > 0 && sim.cubeResults.before && (
              <div className={styles.resultsContainer}>
                <h3 className={styles.resultsTitle}>
                  {sim.cubeResults.gradeUpOccurred ? '등급이 상승했습니다! 적용할 옵션을 선택해주세요.' : '재설정된 잠재능력 중 적용할 옵션을 선택해 주세요.'}
                </h3>
                ...
                <div className={styles.rerollButtonContainer}>
                   <div className={styles.rerollInfo}>ⓘ 재설정을 누를 경우 자동으로 BEFORE 옵션이 선택됩니다.</div>
                   <button className={styles.rerollButton} onClick={sim.handleCubeReroll}>재설정 1회</button>
                </div>
              </div>
            )}
